...
type SecurityGroupLister interface {
	...
	List(selector labels.Selector) (ret []*fabricv1.SecurityGroup, err error)
	// Get retrieves the SecurityGroup from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*fabricv1.SecurityGroup, error)
...
type securityGroupLister struct {
	listers.ResourceIndexer[*fabricv1.SecurityGroup]
}
...
func NewSecurityGroupLister(indexer cache.Indexer) SecurityGroupLister {
	return &securityGroupLister{listers.New[*fabricv1.SecurityGroup](indexer, fabricv1.Resource("securitygroup"))}
}
