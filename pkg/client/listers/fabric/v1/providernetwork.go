...
type ProviderNetworkLister interface {
	...
	List(selector labels.Selector) (ret []*fabricv1.ProviderNetwork, err error)
	// Get retrieves the ProviderNetwork from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*fabricv1.ProviderNetwork, error)
...
type providerNetworkLister struct {
	listers.ResourceIndexer[*fabricv1.ProviderNetwork]
}
...
func NewProviderNetworkLister(indexer cache.Indexer) ProviderNetworkLister {
	return &providerNetworkLister{listers.New[*fabricv1.ProviderNetwork](indexer, fabricv1.Resource("providernetwork"))}
}
