		...
		},
		{
			...
			subnet: fabricv1.Subnet{
				...
				Spec: fabricv1.SubnetSpec{
					...
					Provider:    OvnProvider,
					GatewayType: fabricv1.GWDistributedType,
					NatOutgoingPolicyRules: []fabricv1.NatOutgoingPolicyRule{{
						Match:  fabricv1.NatOutGoingPolicyMatch{SrcIPs: "2001:db8::/32,192.168.0.1/24", DstIPs: "2001:db8::/32"},
						Action: "ACCEPT",
					}},
				},
			},
	...
	tests := []struct {
	...
	}{
		{
			...
			subnet: fabricv1.Subnet{
				Spec: fabricv1.SubnetSpec{
					NatOutgoingPolicyRules: []fabricv1.NatOutgoingPolicyRule{{
						Match: fabricv1.NatOutGoingPolicyMatch{
							SrcIPs: "10.0.0.0/24",
	...
	tests := []struct {
	...
	}{
		{
			...
			vpc: &fabricv1.Vpc{
				Spec: fabricv1.VpcSpec{
					...
					VpcPeerings: []*fabricv1.VpcPeering{{
		...
		},
		{
			...
			vpc: &fabricv1.Vpc{
				Spec: fabricv1.VpcSpec{
					EnableExternal: true,
					DynamicRouting: &fabricv1.VpcDynamicRouting{Enabled: true},
				},
			},
			wantErr: true,
		...
		},
		{
			...
			vpc: &fabricv1.Vpc{
				Spec: fabricv1.VpcSpec{
					...
					DynamicRouting: &fabricv1.VpcDynamicRouting{
						Enabled:      true,
						Redistribute: []fabricv1.RedistributeType{"ospf"},
					},
				},
			},
			wantErr: true,
		},
		{
			...
			vpc: &fabricv1.Vpc{
				Spec: fabricv1.VpcSpec{
					...
					DynamicRouting: &fabricv1.VpcDynamicRouting{
						Enabled:      true,
						Redistribute: []fabricv1.RedistributeType{fabricv1.RedistributeNAT, fabricv1.RedistributeNAT},
					},
				},
			},
			wantErr: true,
		...
		},
		{
			...
			vpc: &fabricv1.Vpc{
				Spec: fabricv1.VpcSpec{
					DynamicRouting: &fabricv1.VpcDynamicRouting{Enabled: false},
				},
			},
			wantErr: false,
		},
...
func TestValidateVpcVrfID(t *testing.T) {
	dynamicRouting := func(vrfID uint32) *fabricv1.VpcDynamicRouting {
		return &fabricv1.VpcDynamicRouting{
			Enabled:      true,
			Redistribute: []fabricv1.RedistributeType{fabricv1.RedistributeNAT},
			VrfID:        vrfID,
		}
	}
	existing := []fabricv1.Vpc{
		{ObjectMeta: metav1.ObjectMeta{Name: "vpc-a"}, Spec: fabricv1.VpcSpec{DynamicRouting: dynamicRouting(1001)}},
		{ObjectMeta: metav1.ObjectMeta{Name: "vpc-off"}, Spec: fabricv1.VpcSpec{DynamicRouting: &fabricv1.VpcDynamicRouting{VrfID: 1002}}},
	}

	duplicate := &fabricv1.Vpc{ObjectMeta: metav1.ObjectMeta{Name: "vpc-b"}, Spec: fabricv1.VpcSpec{DynamicRouting: dynamicRouting(1001)}}
	if err := ValidateVpcVrfID(duplicate, existing); err == nil {
		t.Error("expected a conflict with the vrf id of vpc-a")
	}

	free := &fabricv1.Vpc{ObjectMeta: metav1.ObjectMeta{Name: "vpc-b"}, Spec: fabricv1.VpcSpec{DynamicRouting: dynamicRouting(1003)}}
	if err := ValidateVpcVrfID(free, existing); err != nil {
		t.Errorf("expected a free vrf id to pass, got %v", err)
	}

	reuseOfDisabled := &fabricv1.Vpc{ObjectMeta: metav1.ObjectMeta{Name: "vpc-b"}, Spec: fabricv1.VpcSpec{DynamicRouting: dynamicRouting(1002)}}
	...
	itself := &fabricv1.Vpc{ObjectMeta: metav1.ObjectMeta{Name: "vpc-a"}, Spec: fabricv1.VpcSpec{DynamicRouting: dynamicRouting(1001)}}
