...
func (c *Controller) exportSubnetAvailableIPsGauge(subnet *fabricv1.Subnet) {
...
func (c *Controller) exportSubnetUsedIPsGauge(subnet *fabricv1.Subnet) {
	var usingIPs float64
	if subnet.Spec.Protocol == fabricv1.ProtocolIPv6 {
...
func (c *Controller) exportCentralizedSubnetInfo(subnet *fabricv1.Subnet) {
...
func (c *Controller) exportSubnetIPAMInfo(subnet *fabricv1.Subnet) {
...
func (c *Controller) exportSubnetIPAssignedInfo(subnet *fabricv1.Subnet) {
	ipamSubnet, ok := c.ipam.Subnets[subnet.Name]
	if !ok {
		klog.Errorf("failed to get subnet %s in ipam", subnet.Name)
		return
	...
	if subnet.Spec.Protocol == fabricv1.ProtocolIPv4 || subnet.Spec.Protocol == fabricv1.ProtocolDual {
		for ip, pod := range ipamSubnet.V4IPToPod {
			metricSubnetIPAssignedInfo.WithLabelValues(subnet.Name, ip, pod).Set(1)
		}
	}
	if subnet.Spec.Protocol == fabricv1.ProtocolIPv6 || subnet.Spec.Protocol == fabricv1.ProtocolDual {
