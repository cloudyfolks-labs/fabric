...
func (c *Controller) handleDeleteVM(key string) error {
	...
	for _, port := range ports {
		if err := c.config.FabricClient.FabricV1().IPs().Delete(context.Background(), port.Name, metav1.DeleteOptions{}); err != nil {
...
func (c *Controller) handleAddOrUpdateVMIMigration(key string) error {
	...
	lsps, err := c.OVNNbClient.ListNormalLogicalSwitchPorts(false, map[string]string{"pod": fmt.Sprintf("%s/%s", vmi.Namespace, vmi.Name)})
	if err != nil {
