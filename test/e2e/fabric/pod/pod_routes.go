...
var _ = framework.SerialDescribe("[group:pod]", func() {
	...
	framework.ConformanceIt("should support north gateway via pod annotation", func() {
		...
		podClient.CreateSync(framework.MakePod(namespaceName, podName, nil, annotations, f.FabricImage, cmd, nil))
		...
		framework.ExpectNoError(err)
		err = f.FabricClientSet.FabricV1().IPs().Delete(context.Background(), fmt.Sprintf("%s.%s", pod.Name, pod.Namespace), metav1.DeleteOptions{})
	})
