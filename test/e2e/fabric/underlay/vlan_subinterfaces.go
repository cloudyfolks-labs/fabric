...
var _ = framework.SerialDescribe("[group:underlay]", func() {
	...
	framework.ConformanceIt(`should create vlan subinterface when autoCreateVlanSubinterfaces is true`, func() {
		...
		for _, node := range readyKindNodes {
			...
			framework.ExpectTrue(isFabricAutoCreatedInterface(kindNodeMap, nodeName, nodeIface), fmt.Sprintf("VLAN subinterface %s should be created by fabric on node %s", nodeIface, nodeName))
		}
	...
	framework.ConformanceIt(`should isolate subinterfaces across multiple provider networks`, func() {
		...
		for _, node := range readyKindNodes {
			...
			framework.ExpectTrue(isFabricAutoCreatedInterface(kindNodeMap, nodeName, nodePn1Interface), fmt.Sprintf("VLAN subinterface %s should be created by fabric on node %s", nodePn1Interface, nodeName))

			nodePn2Interface := nodeInterfaceNameFor(nodeName, pn2Interface, nodeInterfaces)
			framework.ExpectFalse(isFabricAutoCreatedInterface(kindNodeMap, nodeName, nodePn2Interface), fmt.Sprintf("Base interface %s on node %s should not be modified by fabric when autoCreateVlanSubinterfaces is false", nodePn2Interface, nodeName))
			...
			framework.ExpectTrue(isFabricAutoCreatedInterface(kindNodeMap, nodeName, nodePn3Interface), fmt.Sprintf("VLAN subinterface %s should be created by fabric on node %s", nodePn3Interface, nodeName))
		}
	...
	framework.ConformanceIt(`should handle subinterfaces edge cases properly`, func() {
		...
		ginkgo.By("should not create subinterface for non-VLAN interface name")
		{
			...
			for _, node := range readyKindNodes {
				...
				if exists {
					framework.ExpectFalse(isFabricAutoCreatedInterface(kindNodeMap, nodeName, nodeIface), fmt.Sprintf("Interface %s on node %s should not be a fabric created subinterface", nodeIface, nodeName))
				}
			}
...
func isFabricAutoCreatedInterface(nodeExecMap map[string]kind.Node, nodeName, interfaceName string) bool {
	output, ok := interfaceOutput(nodeExecMap, nodeName, interfaceName)
