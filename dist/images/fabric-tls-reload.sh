...
case "$COMPONENT" in
  ovs)
    TLS_HASH_FILE=${TLS_HASH_FILE:-/tmp/fabric-tls.hash}
    ;;
  ovn-central)
    TLS_HASH_FILE=${TLS_HASH_FILE:-/tmp/fabric-central-tls.hash}
    ;;
  *)
...
function tls_files_hash {
  for file in "${TLS_FILES[@]}"; do
    if [[ ! -s "$file" ]]; then
      echo "fabric TLS file $file is missing or empty" >&2
      return 1
    fi
  done
...
function reload_ovs_tls {
  echo "fabric TLS files changed, restarting ovn-controller"
...
function reload_ovn_central_tls {
  echo "fabric TLS files changed, reloading ovn-central TLS"
...
function check_tls_once {
  ...
  exec {lock_fd}>&-
  echo "failed to reload fabric TLS files, will retry" >&2
  return 1
}
