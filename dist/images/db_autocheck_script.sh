...
restoreNB(){
  ...
  kubectl exec -it -n $FABRIC_NS ${podNameArray[0]} -- ovsdb-tool cluster-to-standalone  /etc/ovn/ovnnb_db_standalone.db  /etc/ovn/ovnnb_db.db
  ...
  kubectl exec -it -n $FABRIC_NS ${podNameArray[0]} -- mv /etc/ovn/ovnnb_db_standalone.db /etc/ovn/ovnnb_db.db
...
CENTRAL_PODS=$(kubectl get pod -n $FABRIC_NS | grep ovn-central | awk '{print $1}')
<< "COMMENT"
for POD in $CENTRAL_PODS
do
  echo "pod db status for $POD"
  kubectl exec "$POD" -n $FABRIC_NS -c ovn-central -- ovn-appctl -t /var/run/ovn/ovnnb_db.ctl cluster/status OVN_Northbound
  kubectl exec "$POD" -n $FABRIC_NS -c ovn-central -- ovn-appctl -t /var/run/ovn/ovnnb_db.ctl ovsdb-server/get-db-storage-status OVN_Northbound
  echo ""
done
COMMENT
...
for POD in $CENTRAL_PODS
do
  DBSTATUS=$(kubectl exec "$POD" -n $FABRIC_NS -c ovn-central -- ovn-appctl -t /var/run/ovn/ovnnb_db.ctl ovsdb-server/get-db-storage-status OVN_Northbound)
  if ! [[ $DBSTATUS =~ "ok" ]]; then
...
for POD in $CENTRAL_PODS
do
  DBSTATUS=$(kubectl exec "$POD" -n $FABRIC_NS -c ovn-central -- ovn-appctl -t /var/run/ovn/ovnsb_db.ctl ovsdb-server/get-db-storage-status OVN_Southbound)
  if ! [[ $DBSTATUS =~ "ok" ]]; then
...
CENTRAL_IPS=$(kubectl get pod -n $FABRIC_NS -o wide | grep ovn-central | awk '{print $6}')

match=0
NBEPS=$(kubectl get ep -n $FABRIC_NS ovn-nb | awk '{print $2}')
...
match=0
SBEPS=$(kubectl get ep -n $FABRIC_NS ovn-sb | awk '{print $2}')
for IP in $CENTRAL_IPS
do
  if [[ $SBEPS =~ $IP ]]; then
    match=$((match+1))
