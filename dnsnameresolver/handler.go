...
func (resolver *DNSNameResolver) updateResolvedNamesSuccess(
	...
	retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if newResolverObj.APIVersion == "" {
			newResolverObj.APIVersion = "fabric.cloudyfolks.io/v1"
		}
...
func isMatchingResolvedName(
	ipTTLs map[string]int32,
	resolvedName kubeovnapiv1.DNSNameResolverResolvedName,
) bool {
...
func addUpdateResolvedNameIPTTLs(
	...
	for ip, ttl := range ipTTLs {
		if !matchedIPTTLs.Has(ip) {
			resolvedAddress := kubeovnapiv1.DNSNameResolverResolvedAddress{
				IP:             ip,
				TTLSeconds:     ttl,
...
func addResolvedName(
	...
	resolvedName := kubeovnapiv1.DNSNameResolverResolvedName{
		DNSName:            kubeovnapiv1.DNSName(dnsName),
	...
	if isWildcard(dnsName) {
		...
		newResolverObj.Status.ResolvedNames = append([]kubeovnapiv1.DNSNameResolverResolvedName{resolvedName}, newResolverObj.Status.ResolvedNames...)
	} else {
